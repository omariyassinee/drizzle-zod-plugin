...
test("generates valid Zod schemas for PostgreSQL, MySQL, and SQLite complex edge cases", async () => {
	...
	// Write generated code to temporary ESM file and import it to test runtime Zod validation
	const tmpDir = resolve(process.cwd(), "node_modules/.tmp-test");
	mkdirSync(tmpDir, { recursive: true });
	const tmpFile = resolve(tmpDir, `gen-${Date.now()}.mjs`);
